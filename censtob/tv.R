...
tv_serv <- function(input, output,session) {
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      sliderInput(ns("beta"), "Beta Value:", min = -3, max = 3, value = 0, step = 0.1),
      sliderInput(ns("threshold"), "Censoring Threshold:", min = -3, max = 3, value = 3, step = 0.1),
      radioButtons(ns("censoring_type"), "Censoring Type:", choices = c("Below" = "below", "Above" = "above"), selected = "above")
    ),

    mainPanel(
      fluidRow(
        column(6, plotOutput(ns("scatterPlot"))),
        column(6, plotOutput(ns("cdfPlot")))
      )
    )
  )
)
